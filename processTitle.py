...
# count number of each letter in words
def count_letters(doc):
    letter_counts = []
    for word in doc:
        word_letters = {"a": 0, "b": 0, "c": 0, "d": 0, "e": 0, "f": 0, "g": 0, "h": 0, "i": 0, "j": 0, "k": 0, "l": 0,
        ...
        for wordNum, word in enumerate(word_list):
            if not word[4]:
                ai_response = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[{"role": "user", "content": f"Give a word related to: {word[1]}. Only respond with the lowercase word, no punctuation"}]).choices[0].message.content
            else:
                ai_response = ""
                wordDB = words(
                    word_num=word[0],
